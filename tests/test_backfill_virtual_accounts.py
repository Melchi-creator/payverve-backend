...
def make_user(ngn, tag, with_kyc):
    user = UserModel(first_name=tag.title(), last_name='Test',
                     ...
                     mobile_number=f'80100000{len(tag):02d}',
...
def main():
    ...
    with app.app_context():
        ...
        db.session.commit()
        before_done = already_provisioned(done_wallet).account_number

        check('setup: accounts on record', VirtualAccountNumberModel.query.count(), 1)

        # Dry run must change nothing.
        dry = run_backfill(ngn, apply_changes=False)
        check('dry run provisions nothing', VirtualAccountNumberModel.query.count(), 1)
        check('  counts the one already done', dry['already had one'], 1)
        ...
        check('  skips the one already done', applied['already had one'], 1)
        ...
        check('ready wallet now has an account',
              already_provisioned(ready_wallet) is not None, True)
        check('  and is active', ready_wallet.is_active, True)
        check('waiting wallet still has none',
              already_provisioned(WalletModel.query.filter_by(
                  user_id=waiting.id).first()), None)
        check('already-done account untouched',
              already_provisioned(done_wallet).account_number, before_done)
        check('total accounts on record', VirtualAccountNumberModel.query.count(), 2)

        # Re-running is safe.
        second = run_backfill(ngn, apply_changes=True)
        check('re-run provisions nothing new', second['provisioned'], 0)
        check('  everyone provisioned is skipped', second['already had one'], 2)
        check('  accounts unchanged', VirtualAccountNumberModel.query.count(), 2)
